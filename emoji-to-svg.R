...
# Use install.packages() if not installed already
library(polite)
library(rvest)
library(purrr)
library(magick)
...
ep_bow <- bow(
  ...
  user_agent = "M Dray <https://www.matt-dray.com>"  # supply your details
)
...
walk(png_paths[c(82, 19, 55)], ~pixelate_emoji(.x) |> plot_emoji())
dev.off()
